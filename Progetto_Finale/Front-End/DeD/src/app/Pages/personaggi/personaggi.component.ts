...
export class PersonaggiComponent {
  ...
  constructor(
    private personaggioService: PersonaggiService,
  ...
  RedirectPersonaggio(personaggioId: number) {
    this.router
      .navigate(['/infoPersonaggio', personaggioId])
      .then((success) => {
        if (success) {
        } else {
  ...
  elimina(Id: number) {
    this.personaggioService.EliminaPers(Id).subscribe(
      (response) => {
      ...
      (error) => {
        console.error(
          `Errore durante l'eliminazione del personaggio: ${JSON.stringify(
            error
          )}`
        );
      }
  ...
  Modifica = (personaggioId: number) => {
    this.router
      .navigate(['/modificaPersonaggio', personaggioId])
      .then((success) => {
        if (success) {
        } else {
          console.error('Redirect fallito');
