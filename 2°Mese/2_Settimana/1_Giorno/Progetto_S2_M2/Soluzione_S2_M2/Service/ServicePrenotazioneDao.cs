...
namespace Soluzione_S2_M2.Service
{
    ...
    {
        public ServicePrenotazioneDao(IConfiguration configuration)
            : base(configuration) { }

        private const string CraeFormPren =
            @"INSERT INTO Prenotazioni(DataPrenotazione, DataInizio, DataFine, Caparra, Importo, MezzaPensione, PensioneCompleta, PernottamentoPrimaColazione, IdCamera, IdClienti)
        ...
        private const string GetAllPren =
            "SELECT * FROM Prenotazioni AS P JOIN Cliente AS C ON P.IdClienti = C.IdCliente";
        private const string GetAllPens = "SELECT * FROM Prenotazioni AS P JOIN Cliente AS C ON P.IdClienti = C.IdCliente WHERE PensioneCompleta = 1";
        ...
        public void CreaFormPreno(Prenotazioni P)
        {
            try
            {
                ...
                if (
                    P.DataInizio < new DateTime(1753, 1, 1)
                    || P.DataFine > new DateTime(9999, 12, 31)
                )
                {
                    throw new ArgumentOutOfRangeException(
                        "La data è fuori dall'intervallo valido."
                    );
                }
                ...
                command.Parameters.Add(
                    new SqlParameter("@DataPrenotazione", SqlDbType.DateTime2)
                    {
                        Value = P.DataPrenotazione
                    }
                );
                command.Parameters.Add(
                    new SqlParameter("@DataInizio", SqlDbType.DateTime2) { Value = P.DataInizio }
                );
                command.Parameters.Add(
                    new SqlParameter("@DataFine", SqlDbType.DateTime2) { Value = P.DataFine }
                );
                ...
                command.Parameters.Add(
                    new SqlParameter("@PernottamentoPrimaColazione", P.PernottamentoPrimaColazione)
                );
        ...
        public IEnumerable<UPrenotazioniCliente> CreaPreno()
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ListaPrenotazioni.Add(CreaUPren(reader));
                }
            ...
            catch (Exception ex)
            {
                ...
                throw new Exception(
                    "Errore durante la creazione delle prenotazioni: " + ex.Message,
                    ex
                );
            }
        }

        public UPrenotazioniCliente CreaUPren(DbDataReader reader)
        {
            return new UPrenotazioniCliente
            {
                ...
                PernottamentoPrimaColazione = reader.GetBoolean(
                    reader.GetOrdinal("PernottamentoPrimaColazione")
                ),
        ...
        public IEnumerable<UPrenotazioniCliente> AllPrenPensione()
        {
            ...
            try
            {
                var _connecition = GetConnection();
                var command = GetCommand(GetAllPens);
                _connecition.Open();
                var reader = command.ExecuteReader();
                while (reader.Read())
                {
